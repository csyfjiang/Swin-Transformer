...
def softmax_T(x, temperature=1.0):
    # Ensure the temperature is a positive value
    assert temperature > 0, "Temperature must be positive"
    
    # Adjust the tensor values by the temperature
    x = x / temperature
    
    # Compute the softmax
    soft_x = torch.nn.functional.softmax(x, dim=-1)  
...
class SwinTransformerBlock_up(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, data_type=0):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.num_heads = num_heads
        ...
        mlp_hidden_dim = int(dim * mlp_ratio)
        num_experts = 3
        self.experts = nn.ModuleList([Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop) for _ in range(num_experts)])
           
        self.gating_network = AttentionGating(self.dim, num_experts)
    ...
    def forward(self, x, data_type=0):
        H, W = self.input_resolution
        B, L, C = x.shape
        assert L == H * W, "input feature has wrong size"
        ...
        # Priori weights based on data_type
        prior_weights = torch.zeros_like(expert_weights)
        ...
        combined_weights = gamma * prior_weights + (1 - gamma) * expert_weights
        
        combined_weights = softmax_T(combined_weights, temperature=10.0)
        #print(f"combined_weights shape:{combined_weights.shape}, \n combined_weights ={combined_weights}")
        
        expert_outputs = []
        for i, expert in enumerate(self.experts):
            #print(f"x before expert in swin transformer block:{x}")
            expert_output = expert(x)
            #print(f"x after expert in swin transformer block:{expert_output}")
            expert_weight = combined_weights[:, :, i:i + 1]
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, upsample=None, use_checkpoint=False, data_type=0):
...
class TransNuSeg(nn.Module):
    ...
    def forward_up_features(self, seg_mask,edge_mask, seg_mask_downsample,edge_mask_downsample):
        ...
        for inx in range(len(self.layers_up)):
            if inx == 0: # in the first layer of decoder, both seg and edge mask pass the same decoder to reduce the number of parameters 

                seg_mask = self.layers_up[inx](seg_mask, data_type=0)
                cluster_mask = edge_mask
                
                edge_mask = self.layers_up[inx](edge_mask, data_type=1) # edge mask is also cluster mask
                cluster_edge = self.layers_up[inx](cluster_mask, data_type=2) # edge mask is also cluster mask
...
#                 print("edge_mask_downsample[3-inx].shape ==== ", edge_mask_downsample[3-inx].shape)
#                 print("edge_mask_downsample.shapes ==== ", [_.shape for _ in edge_mask_downsample])
#                 print("******************")
                
                edge_mask = torch.cat([edge_mask,edge_mask_downsample[3-inx]],-1)
                edge_mask = self.concat_back_dim[inx](edge_mask)
                edge_mask = self.layers_up[inx](edge_mask, data_type=1)
                ...
                cluster_edge = self.layers_up[inx](cluster_edge, data_type=2)

        seg_mask = self.norm_up(seg_mask)
        edge_mask = self.norm_up(edge_mask)  # B L C
